...
app.get("/auth/check-number", async (c) => {
  ...
  const { data: reserved, error } = await supabase.from("reserved_anonymous_numbers").select("anonymous_number, claimed_by_profile_id").eq("anonymous_number", anonymousNumber).maybeSingle();
  if (error) {
    return c.json({ error: error.message }, 500);
  }
  if (!reserved) {
    const { error: insertError } = await supabase.from("reserved_anonymous_numbers").insert({ anonymous_number: anonymousNumber, notes: "auto-reserved" });
    if (insertError) {
...
app.post("/auth/register", async (c) => {
  ...
  const supabase = getSupabase(c.env);
  const { data: reserved, error: reservedError } = await supabase.from("reserved_anonymous_numbers").select("anonymous_number, claimed_by_profile_id").eq("anonymous_number", anonymousNumber).maybeSingle();
  if (reservedError) {
    return c.json({ error: reservedError.message }, 500);
  }
  if (!reserved) {
    const { error: reserveCreateError } = await supabase.from("reserved_anonymous_numbers").insert({ anonymous_number: anonymousNumber, notes: "auto-reserved" });
  ...
  }).select("id, anonymous_number, age").single();
  if (createError || !createdProfile) {
    return c.json({ error: createError?.message ?? "Profile could not be created." }, 500);
  }
  const { error: reserveUpdateError } = await supabase.from("reserved_anonymous_numbers").update({ claimed_by_profile_id: createdProfile.id, claimed_at: now }).eq("anonymous_number", anonymousNumber);
...
app.post("/auth/login", async (c) => {
  ...
  const supabase = getSupabase(c.env);
  const { data: profile, error } = await supabase.from("profiles").select("id, anonymous_number, password_hash, age").eq("anonymous_number", anonymousNumber).maybeSingle();
  if (error) {
    return c.json({ error: error.message }, 500);
  }
...
app.get("/me/summary", async (c) => {
  ...
  const [{ data: profile }, { count: postsCount }, { count: likesCount }] = await Promise.all([
    supabase.from("profiles").select("anonymous_number, age, created_at").eq("anonymous_number", anonymousNumber).single(),
    supabase.from("posts").select("*", { count: "exact", head: true }).eq("author_anonymous_number", anonymousNumber),
    supabase.from("post_likes").select("*", { count: "exact", head: true }).eq("anonymous_number", anonymousNumber)
  ]);
  return c.json({
    profile,
    stats: {
...
app.get("/posts/feed", async (c) => {
  const supabase = getSupabase(c.env);
  const { data, error } = await supabase.from("posts_feed").select("*").order("created_at", { ascending: false }).limit(50);
  if (error) {
    return c.json({ error: error.message }, 500);
  }
...
app.post("/posts/:postId/like", async (c) => {
  ...
  const anonymousNumber = c.get("anonymousNumber");
  const supabase = getSupabase(c.env);
  const { error } = await supabase.from("post_likes").upsert({ post_id: postId, anonymous_number: anonymousNumber }, { onConflict: "post_id,anonymous_number" });
  if (error) {
    return c.json({ error: error.message }, 500);
  }
...
app.post("/reports", async (c) => {
  ...
  const { data, error } = await supabase.from("reports").insert({
    ...
    review_after: new Date(Date.now() + 3 * 24 * 60 * 60 * 1e3).toISOString()
  }).select("*").single();
  if (error) {
    return c.json({ error: error.message }, 500);
  }
...
function isAllowedOrigin(requestOrigin, allowed) {
...
function sanitizeAnonymousNumber(value) {
  if (!value) {
    return "";
  }
...
__name(sanitizeAnonymousNumber, "sanitizeAnonymousNumber");
async function issueToken(secret, profileId, anonymousNumber) {
  return sign2(
    {
      sub: profileId,
      anonymousNumber,
...
async function requireAuth(c, next) {
  ...
  try {
    const payload = await verify2(authorization.slice(7), c.env.JWT_SECRET, "HS256");
